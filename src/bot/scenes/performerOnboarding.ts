...
const summaryKeyboard = () =>
  Markup.inlineKeyboard([
    ...
    [Markup.button.callback('Изменить описание', 'edit_about')],
    [Markup.button.callback('Изменить фото', 'edit_photo')],
    [Markup.button.callback('Изменить голос', 'edit_voice')],
    [Markup.button.callback('Изменить реквизиты', 'edit_pay')],
    [Markup.button.callback('Отправить', 'po_submit')],
    [Markup.button.callback('Отмена', 'wiz_cancel')],
  ]);

const showSummary = async (ctx: Scenes.WizardContext & { session: any }) => {
  const st = ctx.wizard.state as PerfWizardState;
  try {
    if (st.photoUrl) await ctx.replyWithPhoto(st.photoUrl.replace('tg:', ''));
  } catch {}
  try {
    if (st.voiceSampleUrl) await ctx.replyWithVoice(st.voiceSampleUrl.replace('tg:', ''));
  } catch {}
  const summary =
    `Проверьте анкету:\n` +
    `Игры: ${st.games.join(', ')}\n` +
  ...
  await ctx.reply(summary, summaryKeyboard());
};
...
export const performerOnboarding = new Scenes.WizardScene<Scenes.WizardContext & { session: any }>(
  ...
  async (ctx) => {
    ...
    // handle game selection edits
    if (st.editField === 'games' && st.stage === 'select_games') {
      const data = (ctx.update as any)?.callback_query?.data as string | undefined;
      if (!data) return;
      if (data === 'po_done') {
        st.editField = undefined;
        st.stage = undefined;
        await ctx.answerCbQuery?.('Сохранено');
        await showSummary(ctx);
        return;
    ...
    if (data) {
      switch (data) {
        ...
        case 'edit_pay':
          st.editField = 'pay';
          await ctx.answerCbQuery?.();
          await ctx.reply('Укажите реквизиты для получения оплаты.');
          return;
        case 'po_submit': {
    ...
    // handle text/photo/audio edits
    if (st.editField === 'price') {
      const price = Number(
        ctx.message && 'text' in ctx.message ? (ctx.message.text || '').replace(/[^0-9]/g, '') : '0',
      );
      if (!price || price <= 0) {
