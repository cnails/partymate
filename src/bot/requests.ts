...
export const registerRequestFlows = (bot: Telegraf) => {
  ...
  bot.on("callback_query", async (ctx, next) => {
    ...
    if (data.startsWith("req_accept:")) {
      ...
      // Рассчитаем стоимость заявки исходя из времени
      const pricePerHour =
        req.performer.performerProfile?.pricePerHour ?? 0;
      const amount = Math.round((pricePerHour * req.durationMin) / 60);
