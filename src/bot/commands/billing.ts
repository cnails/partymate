    ...
    await ctx.reply(
      [
        ...
        boostActive
          ...
          : '• Буст: нет (можно подключить в любой момент)',
        '',
        'Зачем нужен буст?',
        'Буст — это ускоритель видимости: он поднимает вашу анкету выше в выдаче на 7 или 14 дней.',
        '',
        'Про тарифы:',
        '• BASE — доступно описание анкеты',
        '• STANDARD — доступно фото и выше видимость, чем у BASIC.',
        '• PRO — доступно фото, голосовая запись и максимальная видимость в поиске.',
        '',
        'Кто выше в списке?',
        'PRO + буст > PRO > STANDARD + буст > STANDARD > BASIC + буст > BASIC.',
        '',
        'Цены:',
        `• Буст 7д — ${config.billing.BOOST_7D_RUB}₽; 14д — ${config.billing.BOOST_14D_RUB}₽`,
        `• STANDARD 30д — ${config.billing.PLAN_STD_30D_RUB}₽; 90д — ${config.billing.PLAN_STD_90D_RUB}₽`,
        `• PRO 30д — ${config.billing.PLAN_PRO_30D_RUB}₽; 90д — ${config.billing.PLAN_PRO_90D_RUB}₽`,
      ].join('\n'),
      kbMain(),
    );
  });
